...
public class GunshipController : MonoBehaviour
{
    ...
    {
        GameObject cannonBall = Instantiate(cannonballPrefab, cannon.position, quaternion.identity, cannon);
        cannonBall.GetComponent<Rigidbody2D>().AddForce(direction.normalized * cannonballForce, ForceMode2D.Impulse);
    }
}
